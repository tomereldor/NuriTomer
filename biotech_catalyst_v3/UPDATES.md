...
**`ml_dataset_features_20260310_v3.csv`** ← THE ONE FILE TO USE FOR ML TRAINING
- **827 rows × 104 cols** — 4 unsafe rows removed; `feat_endpoint_outcome_score` replaces `feat_endpoint_positive_score`
- Target columns: `target_abs_move_atr` (regression), `target_move_bucket` (5-class), `target_large_move` (binary)
- Feature columns (35 `feat_*`): all pass1 + pass3 features; `feat_date_corrected_flag` added; `feat_endpoint_positive_score` removed
- `row_ready=True`: 814 rows · baseline training filter: `df[row_ready & v_actual_date.notna()]` → **813 rows**
- Generated by `build_ml_ready_features.py` (pass1) → `add_high_value_predictors.py` (pass3) → `completeness_pass.py` (fills) → v3.16 repairs

**`ml_feature_dict_20260310_v3.csv`** — feature dictionary (39 entries, updated for v3.16)
- 39 entries (3 targets + 36 features): name, type, n_valid, n_null, pct_valid
