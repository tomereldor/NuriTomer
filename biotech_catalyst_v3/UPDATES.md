...
**`ml_dataset_features_20260310_v5.csv`** ← THE ONE FILE TO USE FOR ML TRAINING
- **827 rows × 132 cols** — pass-5 timing features adds 9 new deterministic sequence/proximity features
...
**`ml_feature_dict_20260310_v6.csv`** — feature dictionary (69 entries, updated for v3.23)
- 60 entries (3 targets + 55 active features + 1 deprecated + 1 excluded)
- Schema: feature_name, role, feature_group, data_type, source_columns, source_system, calculation_logic, plain_english_definition, why_it_matters, valid_count, null_count, valid_pct, null_pct, validity_rule, notes
- `feat_endpoint_positive_score` marked DEPRECATED; `is_pivotal` marked EXCLUDED with explanation
...
### 2026-03-13 — Pre-event model v3: timing features + fold-safe priors (v3.23)

**Feature dataset:** `ml_dataset_features_20260310_v5.csv` (827 rows × 132 cols, +9 new timing features)
**Feature dict:** `ml_feature_dict_20260310_v6.csv` (69 entries)
**Train table:** `ml_baseline_train_20260312_v2.csv` (813 rows, 64 features + split)
**Report:** `reports/ml_pre_event_v3_report_20260312_v1.md`

**New timing features added (9):**
- `feat_primary_completion_imminent_30d` / `_90d` — binary readout proximity flags
- `feat_completion_recency_bucket` — 6-level categorical (imminent/near/medium/far/past/unknown)
- `feat_time_since_last_company_event` — days since prior ticker event (65% coverage)
- `feat_time_since_last_asset_event` — days since prior same-asset event (24% coverage)
- `feat_asset_event_sequence_num` / `feat_company_event_sequence_num` — event ordinal position
- `feat_recent_company_event_flag` / `feat_recent_asset_event_flag` — recency binary flags

**Fold-safe priors added (6, injected inside CV folds only):**
- Prior mean abs ATR move by: phase, therapeutic superclass, phase×superclass interaction, market-cap bucket
- Prior large-move rate by: phase, therapeutic superclass
- Fit on train fold only; global train fallback for unseen categories; no leakage

**Results (v3 vs v2 baseline):**
- Best model: LogReg | Test ROC-AUC: 0.661 (v2: 0.685) | CV AUC: 0.682 ± 0.129 (v2: 0.735 ± 0.061)
- Timing features appear prominently in importances: `feat_recent_company_event_flag` #1
- Direct comparison slightly unfair: v2 and v3 use different test splits (event_date vs v_actual_date ordering)
- Verdict: timing features add signal but not net lift yet; prior features need more training data

**feat_days_to_study_completion:** Skipped — no `ct_study_completion` column in current dataset.
...
**Previous v4 dict archived:** `archive/`

Rebuilt from scratch using `ml_dataset_features_20260310_v4.csv` as ground truth.
All coverage stats re-derived from actual data (not copy-forwarded from prior versions).

**Schema** (15 fields per entry):
`feature_name · role · feature_group · data_type · source_columns · source_system · calculation_logic · plain_english_definition · why_it_matters · valid_count · null_count · valid_pct · null_pct · validity_rule · notes`

**Major changes vs v4 dict:**
- All 55 active features + 3 targets fully documented with calculation logic and plain-English definitions
- `feat_endpoint_positive_score` → role=**DEPRECATED**, explains replacement by `feat_endpoint_outcome_score`
- `is_pivotal` (raw column) → role=**EXCLUDED**, explains 54% null issue and recommended replacements
- All coverage stats refreshed from v4 dataset directly
- Historical prior columns carry explicit ⚠ TRAIN-ONLY warning in notes
- `feat_therapeutic_superclass` and all pass4 features fully documented
