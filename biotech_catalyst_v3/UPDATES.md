...
**`ml_dataset_features_20260310_v1.csv`** ← THE ONE FILE TO USE FOR ML TRAINING
- **831 rows × 80 cols** — all raw columns preserved + 3 target columns + 12 feature columns
...
- Feature columns (12 `feat_*`): phase, late-stage flag, endpoint score, mesh encoding, log market cap, short squeeze, institutional ownership, volatility, timing, proximity bucket, active-not-recruiting, design quality
- Generated by `build_ml_ready_features.py` from `ml_dataset_mesh_final.csv`

**`ml_feature_dict_20260310_v1.csv`** — feature dictionary
- 15 entries (3 targets + 12 features): name, type, description, source columns, dtype, n_valid, n_null, pct_valid
...
### 2026-03-10 — ML Feature Engineering (v3.13)

**Files added:**
- `ml_dataset_features_20260310_v1.csv` — 831 rows × 80 cols (final ML-ready dataset)
- `ml_feature_dict_20260310_v1.csv` — 15-row feature dictionary
- `build_ml_ready_features.py` — standalone feature engineering script
