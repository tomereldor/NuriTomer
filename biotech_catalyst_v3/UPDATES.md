...
**`ml_dataset_features_20260310_v2.csv`** ← THE ONE FILE TO USE FOR ML TRAINING
- **831 rows × 103 cols** — all raw columns preserved + 3 target columns + 35 feature columns
- Updated in-place by completeness pass (v3.15) — 1,818 cells filled across 8 columns
- Target columns: `target_abs_move_atr` (regression), `target_move_bucket` (5-class), `target_large_move` (binary)
- Feature columns (35 `feat_*`): 12 pass1 features (phase, endpoint, mesh, market, timing, design quality) + 23 pass3 features (company dependency, regulatory state, trial quality, reaction priors)
- Generated by `build_ml_ready_features.py` (pass1) → `add_high_value_predictors.py` (pass3) → `completeness_pass.py` (fills)
- Key coverage after completeness pass: price data 99.9% · endpoint_met 44.3% · is_pivotal 45.6% · phase 93.1% · design_quality 94.3%

**`ml_feature_dict_20260310_v2.csv`** — feature dictionary (coverage updated after completeness pass)
- 38 entries (3 targets + 35 features): name, stage, feature_type, source_columns, transformation_logic, dtype, coverage, missing_count
